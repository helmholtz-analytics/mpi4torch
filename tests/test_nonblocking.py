...
class TestNonBlocking(unittest.TestCase):
    def test_simple_isendirecv(self):
        ...
        req2 = comm.Irecv(torchmpi.JoinDummies(torch.empty_like(tmp),[req.dummy]),(comm.rank+comm.size-1)%comm.size,0)
        res = comm.Wait(torchmpi.JoinDummiesHandle(req,[req2.dummy]))
    ...
    def test_simple_isendrecv(self):
        tmp = torch.rand(10000000, dtype=torch.double).requires_grad_()
        req = comm.Isend(tmp,(comm.rank+1)%comm.size,0)
        res = comm.Recv(torchmpi.JoinDummies(torch.empty_like(tmp),[req.dummy]),(comm.rank+comm.size-1)%comm.size,0)
